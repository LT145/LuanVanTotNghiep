...
export default function RecentlyViewedProducts() {
  ...
  // ❌ Nếu đang load session hoặc không có session → ẩn component
  if (status === "loading") return null;
  if (!session) return null;

  useEffect(() => {
    fetch("/api/products/recently-viewed", { cache: "no-store" })
      .then((res) => res.json())
      .then((data) => {
    ...
    <section className="py-20 bg-secondary">
      <div className="custom-container mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-muted-foreground">Những sản phẩm bạn đã xem gần đây</p>
        </div>
